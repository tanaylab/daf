...
``rows_axis,columns_axis;name`` for 2D data. Such data need only be reported under one name, that is, there is no
requirement to also report the transposed data by the name ``columns_axis,rows_axis;name``. When picking which of the
two to report, ideally the data should be reported such that the result would be in `.ROW_MAJOR` order, if at all
