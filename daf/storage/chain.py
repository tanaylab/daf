...
This serves two purposes. First, it is only a :py:obj:`~daf.storage.interface.StorageReader`, so it protects the
wrapped storage objects against accidental modification, even if they implement
:py:obj:`~daf.storage.interface.StorageWriter` as well. Second, it allows delta-encoding, where data in early storage
