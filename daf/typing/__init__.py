...
without a combinatorical explosion of data types. See `.dtypes` for details on how ``daf`` deals with element data
types.
...
provide ``mypy`` type annotations these days, its use of a catch-all ``numpy.ndarray`` type is not sufficient for
capturing the code intent. Therefore, in all cases, the results of any computation on the data types provided here is
effectively ``Any``, which negates the usefulness of using ``mypy``.
