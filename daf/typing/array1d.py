...
def is_array1d(data: Any, *, dtype: Union[None, str, Collection[str]] = None) -> TypeGuard[Array1D]:
    """
    Check whether some ``data`` is an `.Array1D`, optionally only of some ``dtype``.
...
def be_array1d(data: Any, *, dtype: Union[None, str, Collection[str]] = None) -> Array1D:
    """
    Assert that some ``data`` is an `.Array1D`, optionally only of some ``dtype``, and return it as such for
    ``mypy``.
...
#: Anything that can be used to construct an `.Array1D`.
