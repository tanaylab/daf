...
#: 2-dimensional ``numpy`` array in `.ROW_MAJOR` layout.
ArrayInRows = NewType("ArrayInRows", np.ndarray)


def is_array_in_rows(data: Any, *, dtype: Optional[Union[str, Collection[str]]] = None) -> TypeGuard[ArrayInRows]:
    """
    Check whether some ``data`` is an `.ArrayInRows`, optionally only of some ``dtype``.
...
def be_array_in_rows(data: Any, *, dtype: Optional[Union[str, Collection[str]]] = None) -> ArrayInRows:
    """
    Assert that some ``data`` is an `.ArrayInRows`, optionally only of some ``dtype``, and return it as such for
    ``mypy``.
...
def is_array_in_columns(data: Any, *, dtype: Optional[Union[str, Collection[str]]] = None) -> TypeGuard[ArrayInColumns]:
    """
    Check whether some ``data`` is an `.ArrayInColumns`, optionally only of some ``dtype``.
...
def be_array_in_columns(data: Any, *, dtype: Optional[Union[str, Collection[str]]] = None) -> ArrayInColumns:
    """
    Assert that some ``data`` is an `.ArrayInColumns`, optionally only of some ``dtype``, and return it as such
    for ``mypy``.
...
def is_array2d(
    ...
    """
    Check whether some ``data`` is an `.Array2D`, optionally only of some ``dtype``, optionally only of some ``layout``.
...
def be_array2d(
    ...
    *,
    ...
    """
    Assert that some ``data`` is an `.Array2D` optionally only of some ``dtype``, optionally of only of a specific
...
#: Anything that can be used to construct an `.Array2D`.
