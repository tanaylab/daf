...
* `.Frame` is a ``pandas.DataFrame`` which contains an ``Array2D`` of a single element type, with indices of names for
...
.. note::
    ...
    The `.Dense` type should be used in computations with **great care**, as some operations are subtly different for 2D
    ``numpy.ndarray`` and ``pandas.DataFrame``. It is typically better to use one of the concrete types instead.
"""
...
#: Any dense 2D data in `.ROW_MAJOR` layout.
DenseInRows = Union["_array2d.ArrayInRows", _frames.FrameInRows]
...
def is_dense_in_rows(data: Any, *, dtype: Optional[Union[str, Collection[str]]] = None) -> TypeGuard[DenseInRows]:
    ...
    """
    return _array2d.is_array_in_rows(data, dtype=dtype) or _frames.is_frame_in_rows(data, dtype=dtype)
...
#: Any dense 2D data in `.COLUMN_MAJOR` layout.
DenseInColumns = Union["_array2d.ArrayInColumns", _frames.FrameInColumns]
...
def is_dense_in_columns(data: Any, *, dtype: Optional[Union[str, Collection[str]]] = None) -> TypeGuard[DenseInColumns]:
    ...
    By default, checks that the data type is one of `.ALL_DTYPES`.
    """
    return _array2d.is_array_in_columns(data, dtype=dtype) or _frames.is_frame_in_columns(data, dtype=dtype)
...
#: Any dense 2D data in either `.ROW_MAJOR` or `.COLUMN_MAJOR` layout.
Dense = Union["_array2d.Array2D", _frames.Frame]
...
def is_dense(
    ...
    return _array2d.is_array2d(data, dtype=dtype, layout=layout) or _frames.is_frame(data, dtype=dtype, layout=layout)
