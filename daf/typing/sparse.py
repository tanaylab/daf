"""
The types here describe a sparse compressed ``scipy.sparse.csr_matrix`` and ``scipy.sparse.csc_matrix`` data, which are
two supported ways to store 2D data in ``daf``.

In theory it should have been possible to store sparse data inside a ``pandas.DataFrame``, but in practice this fails in
various ways, so ``daf`` only stores such frames if they contain dense (that is, :py:const:`daf.typing.array2d.Array2D`)
data.

.. note::

    Other sparse formats (e.g. ``scipy.sparse.coo_matrix``) can't be stored in ``daf``. This allows ``daf`` to require
    that all stored data is in either :py:const:`daf.typing.layouts.ROW_MAJOR` or
    :py:const:`daf.typing.layouts.COLUMN_MAJOR` layout, which greatly simplifies the code accessing the data and also
...
def is_sparse_in_rows(data: Any, *, dtype: Optional[Union[str, Collection[str]]] = None) -> TypeGuard[SparseInRows]:
    """
    Check whether some ``data`` is a :py:const:`SparseInRows`, optionally only of some ``dtype``.
...
def be_sparse_in_rows(data: Any, *, dtype: Optional[Union[str, Collection[str]]] = None) -> SparseInRows:
    """
    Assert that some ``data`` is a :py:const:`SparseInRows`, optionally only of some ``dtype``, and return it as such
    for ``mypy``.
...
def is_sparse_in_columns(
    ...
    """
    Check whether some ``data`` is a :py:const:`SparseInColumns`, optionally only of some ``dtype``.
...
def be_sparse_in_columns(data: Any, *, dtype: Optional[Union[str, Collection[str]]] = None) -> SparseInColumns:
    """
    Assert that some ``data`` is a :py:const:`SparseInColumns`, optionally only of some ``dtype``, and return it as such
    for ``mypy``.
...
def is_sparse(
    ...
    """
    Check whether some ``data`` is a :py:const:`Sparse`, optionally only of some ``dtype``, optionally only of some
    ``layout``.
...
def be_sparse(
    ...
    """
    Assert that some ``data`` is a :py:const:`Sparse` optionally only of some ``dtype``, optionally of some ``layout``,
