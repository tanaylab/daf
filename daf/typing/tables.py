...
data type (that is, a `.Frame`), hence we call this type `.Table` (which admittedly isn't a great name). Of course,
``pandas`` does not make this distinction, so even if/when it provides ``mypy`` annotations, we'd still need to set up
the types here (similar to the problem with ``numpy.ndarray``).
...
TableInRows = NewType("TableInRows", _fake_pandas.PandasFrame)


def is_table_in_rows(data: Any, *, dtype: Optional[Union[str, Collection[str]]] = None) -> TypeGuard[TableInRows]:
    """
    Check whether some ``data`` is a `.TableInRows`, optionally only of some ``dtype``.
...
def be_table_in_rows(data: Any, *, dtype: Optional[Union[str, Collection[str]]] = None) -> TableInRows:
    """
    Assert that some ``data`` is a `.TableInRows`, optionally only of some ``dtype``, and return it as such for
    ``mypy``.
...
TableInColumns = NewType("TableInColumns", _fake_pandas.PandasFrame)


def is_table_in_columns(data: Any, *, dtype: Optional[Union[str, Collection[str]]] = None) -> TypeGuard[TableInColumns]:
    """
    Check whether some ``data`` is a `.TableInColumns`, optionally only of some ``dtype``.
...
def be_table_in_columns(data: Any, *, dtype: Optional[Union[str, Collection[str]]] = None) -> TableInColumns:
    """
    Assert that some ``data`` is a `.TableInColumns`, optionally only of some ``dtype``, and return it as such for
    ``mypy``.
...
def is_table(
    ...
    """
    Check whether some ``data`` is a `.Table`, optionally only of some ``dtype``, optionally only of some ``layout``.
...
def be_table(
    ...
    """
    Assert that some ``data`` is a `.Table` optionally only of some ``dtype``, optionally only of some ``layout``, and
