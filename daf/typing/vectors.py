...
* :py:obj:`~daf.typing.array1d.Array1D` is a one-dimensional ``numpy.ndarray``.
* :py:obj:`~daf.typing.series.Series` is a ``pandas.Series`` which combines an ``Array1D`` with an index of names.

The :py:obj:`~Vector` type annotations is their union, that is, allows for "any" 1D data.
...
matrix, you will need to convert it to a dense format (e.g. by calling :py:obj:`~daf.typing.array1d.as_array1d`) before
storing it in ``daf``.

.. note::

    The :py:obj:`~Vector` type should be used in computations with **great care**, as some operations are subtly
...
def is_vector(data: Any, *, dtype: Union[None, str, Collection[str]] = None) -> TypeGuard[Vector]:
    """
    Assert that some ``data`` is an :py:obj:`~Vector`, optionally only of some ``dtype``, and return it as such for
    ``mypy``.
...
def be_vector(data: Any, *, dtype: Union[None, str, Collection[str]] = None) -> Vector:
    """
    Assert that some ``data`` is a :py:obj:`~Vector`, optionally only of some ``dtype``, and return it as such for
    ``mypy``.
